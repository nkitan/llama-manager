...
pub async fn get_target_model_with_host(
    ...
    // Default fallback: get name from active_model_path
...
pub async fn enrich_model(
    ...
    // 1. Setup client with stealth headers
    ...
    // If HF found use case, pre-populate it!
            ...
            if let Ok(response_text) = res.text().await {
                if let Ok(searx_res) = serde_json::from_str::<SearxResponse>(&response_text) {
                    ...
                    if !results.is_empty() {
                        ...
                        // Query Local LLM via chat completions
                        let prompt = format!(
...
mod tests {
    ...
    #[test]
    fn test_parse_model_hierarchy() {
        let h1 = parse_model_hierarchy("supergemma4-26b-uncensored-fast-v2-Q4_K_M.gguf");
        assert_eq!(h1.family, "Gemma");
        assert_eq!(h1.version, "Gemma 4");
        assert!(h1.tags.contains(&"GGUF".to_string()));
        assert!(h1.tags.contains(&"Q4_K_M".to_string()));
        assert!(h1.tags.contains(&"Uncensored".to_string()));

        let h2 = parse_model_hierarchy("qwen3.6-27b-Q4_K_M.gguf");
        assert_eq!(h2.family, "Qwen");
        assert_eq!(h2.version, "Qwen 3.6");
        assert!(h2.tags.contains(&"GGUF".to_string()));
        assert!(h2.tags.contains(&"Q4_K_M".to_string()));

        let h3 = parse_model_hierarchy("qwen3-coder-next-ud-Q2_K_XL.gguf");
        assert_eq!(h3.family, "Qwen");
        assert_eq!(h3.version, "Qwen 3-Next");
        assert!(h3.tags.contains(&"Q2_K_XL".to_string()));

        let h4 = parse_model_hierarchy("ternary-bonsai-8b-Q2_K.gguf");
        assert_eq!(h4.family, "Ternary Bonsai");
        assert_eq!(h4.version, "Ternary Bonsai 8B");

        let h5 = parse_model_hierarchy("rnj-1-latest.gguf");
        assert_eq!(h5.family, "Rnj");
        assert_eq!(h5.version, "Rnj 1");

        let h6 = parse_model_hierarchy("zeta-2-Q8_0.gguf");
        assert_eq!(h6.family, "Zeta");
        assert_eq!(h6.version, "Zeta 2");

        let h7 = parse_model_hierarchy("phi4-q8_0.gguf");
        assert_eq!(h7.family, "Phi");
        assert_eq!(h7.version, "Phi 4");
    }
}
